...
$manager_id_to_update = $_GET['id'] ?? null;

if(!$manager_id_to_update){
    // If no ID is provided, redirect back to view_product_managers or show an error
    header('location:view_product_managers.php');
    exit();
}

// Fetch current manager details
$select_current_manager = $conn->prepare("SELECT manager_id, name, expertise, password FROM `product_manager` WHERE manager_id = ?");
$select_current_manager->execute([$manager_id_to_update]);
$fetch_current_manager = $select_current_manager->fetch(PDO::FETCH_ASSOC);

if(!$fetch_current_manager){
    // Manager not found, redirect
...
if(isset($_POST['update_manager'])){
    ...
    $prev_pass_check = $conn->prepare("SELECT password FROM `product_manager` WHERE manager_id = ?");
    $prev_pass_check->execute([$id]);
    $prev_pass_row = $prev_pass_check->fetch(PDO::FETCH_ASSOC);
    $prev_password_db = $prev_pass_row['password'];


    // Check if name already exists for another manager
    $select_existing_name = $conn->prepare("SELECT manager_id FROM `product_manager` WHERE name = ? AND manager_id != ?");
    $select_existing_name->execute([$name, $id]);
    if($select_existing_name->rowCount() > 0){
        $message[] = 'Name already exists for another Product Manager!';
    } else {
        // Update name and expertise
        $update_manager_details = $conn->prepare("UPDATE `product_manager` SET name = ?, expertise = ? WHERE manager_id = ?");
        $update_manager_details->execute([$name, $expertise, $id]);
        if($update_manager_details->rowCount() > 0) {
            $message[] = 'Product Manager details updated successfully!';
        }
        ...
        if(!empty($_POST['new_pass'])){ // Only update if new password is provided
            if($old_pass != $prev_password_db){
                ...
                $update_password = $conn->prepare("UPDATE `product_manager` SET password = ? WHERE manager_id = ?");
            ...
            if($update_manager_details->rowCount() == 0) { // If details update didn't change anything
                 $message[] = 'No changes made or other error occurred!';
            }
        }
    }

    // After update, re-fetch the manager details to display latest
    $select_current_manager->execute([$manager_id_to_update]);
    $fetch_current_manager = $select_current_manager->fetch(PDO::FETCH_ASSOC);
}
?>
...
<head>
    ...
    <title>FashionGuys | Update Product Manager</title>
...
<section class="update-product-manager mt-5">
    <div class="content-card">
        <div class="card-header">
            <div class="header-icon">
                <i class="fas fa-sitemap"></i>
            </div>
            <h2 class="header-title">Update Product Manager: <?= htmlspecialchars($fetch_current_manager['name']); ?></h2>
            <p class="text-muted">Modify manager details or change password</p>
        </div>
        ...
        <form action="" method="post" class="needs-validation" novalidate>
            <input type="hidden" name="id" value="<?= htmlspecialchars($fetch_current_manager['manager_id']); ?>">
            <div class="mb-4">
                <div class="form-floating">
                    <input type="text"
                           ...
                           value="<?= htmlspecialchars($fetch_current_manager['name']); ?>"
                           required
                           maxlength="20"
                           oninput="this.value = this.value.replace(/\s{2,}/g, ' ')">
                    <label for="name">Name</label>
                    <div class="invalid-feedback">
                        Please enter the manager's name (max 20 characters)
                    </div>
                </div>
            </div>
            ...
            <div class="mb-4">
                <div class="form-floating">
                    <input type="text"
                           ...
                           value="<?= htmlspecialchars($fetch_current_manager['expertise']); ?>"
                           required
                           maxlength="50">
                    <label for="expertise">Expertise</label>
                    <div class="invalid-feedback">
                        Please enter the expertise area (max 50 characters)
                    </div>
                </div>
            </div>
            ...
            <button type="submit"
                ...
                <i class="fas fa-check-circle me-2"></i>Update Product Manager
            </button>
            <div class="text-center mt-3">
                <a href="view_product_managers.php" class="btn btn-secondary">
                    <i class="fas fa-arrow-left me-2"></i>Back to Manager List
                </a>
            </div>
        </form>
    </div>
