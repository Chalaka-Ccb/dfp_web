...
// Get the agent ID from the URL parameter
$agent_id_to_update = $_GET['id'] ?? null;

if(!$agent_id_to_update){
    // If no ID is provided, redirect back to view_delivery_agents or show an error
    header('location:view_delivery_agents.php');
    exit();
}

// Fetch current agent details
$select_current_agent = $conn->prepare("SELECT id, name, phone, password FROM `delivery_agents` WHERE id = ?");
$select_current_agent->execute([$agent_id_to_update]);
$fetch_current_agent = $select_current_agent->fetch(PDO::FETCH_ASSOC);

if(!$fetch_current_agent){
...
if(isset($_POST['update_agent'])){
    ...
    $prev_pass_check = $conn->prepare("SELECT password FROM `delivery_agents` WHERE id = ?");
    ...
    $select_existing_phone = $conn->prepare("SELECT id FROM `delivery_agents` WHERE phone = ? AND id != ?");
    $select_existing_phone->execute([$phone, $id]);
    if($select_existing_phone->rowCount() > 0){
        $message[] = 'Phone number already exists for another delivery agent!';
    } else {
        // Update name and phone
        $update_agent_details = $conn->prepare("UPDATE `delivery_agents` SET name = ?, phone = ? WHERE id = ?");
        $update_agent_details->execute([$name, $phone, $id]);
        if($update_agent_details->rowCount() > 0) {
            $message[] = 'Delivery Agent details updated successfully!';
        }
        ...
        if(!empty($_POST['new_pass'])){ // Only update if new password is provided
            if($old_pass != $prev_password_db){
                ...
                $update_password = $conn->prepare("UPDATE `delivery_agents` SET password = ? WHERE id = ?");
            ...
            // No new password provided, just check if name/phone was updated
            if($update_agent_details->rowCount() == 0) { // If details update didn't change anything
                 $message[] = 'No changes made or other error occurred!';
            }
        }
    }

    // After update, re-fetch the agent details to display latest
    $select_current_agent->execute([$agent_id_to_update]);
    $fetch_current_agent = $select_current_agent->fetch(PDO::FETCH_ASSOC);
}
?>
...
<head>
    ...
    <title>FashionGuys | Update Delivery Agent</title>
...
<section class="update-delivery-agent mt-5">
    <div class="content-card">
        <div class="card-header">
            <div class="header-icon">
                <i class="fas fa-truck-loading"></i>
            </div>
            <h2 class="header-title">Update Delivery Agent: <?= htmlspecialchars($fetch_current_agent['name']); ?></h2>
            <p class="text-muted">Modify agent details or change password</p>
        </div>
        ...
        <form action="" method="post" class="needs-validation" novalidate>
            <input type="hidden" name="id" value="<?= htmlspecialchars($fetch_current_agent['id']); ?>">
            <div class="mb-4">
                <div class="form-floating">
                    <input type="text"
                           ...
                           value="<?= htmlspecialchars($fetch_current_agent['name']); ?>"
                           required
                           maxlength="100"
            ...
            <div class="mb-4">
                <div class="form-floating">
                    <input type="tel"
                           ...
                           value="<?= htmlspecialchars($fetch_current_agent['phone']); ?>"
                           required
                           pattern="[0-9]{10}"
                           maxlength="15"
            ...
            <button type="submit"
                ...
                <i class="fas fa-check-circle me-2"></i>Update Delivery Agent
            </button>
            <div class="text-center mt-3">
                <a href="view_delivery_agents.php" class="btn btn-secondary">
