...
$supplier_id_to_update = $_GET['id'] ?? null;

if(!$supplier_id_to_update){
    // If no ID is provided, redirect back to view_suppliers or show an error
    header('location:view_suppliers.php');
    exit();
}

// Fetch current supplier details
$select_current_supplier = $conn->prepare("SELECT supplier_id, name, phone, email, address, password FROM `suppliers` WHERE supplier_id = ?");
$select_current_supplier->execute([$supplier_id_to_update]);
$fetch_current_supplier = $select_current_supplier->fetch(PDO::FETCH_ASSOC);

if(!$fetch_current_supplier){
    // Supplier not found, redirect
...
if(isset($_POST['update_supplier'])){
    ...
    $prev_pass_check = $conn->prepare("SELECT password FROM `suppliers` WHERE supplier_id = ?");
    ...
    $select_existing_contact = $conn->prepare("SELECT supplier_id FROM `suppliers` WHERE (email = ? OR phone = ?) AND supplier_id != ?");
    $select_existing_contact->execute([$email, $phone, $id]);
    if($select_existing_contact->rowCount() > 0){
        $message[] = 'Email or Phone already exists for another supplier!';
    } else {
        // Update name, phone, email, and address
        $update_supplier_details = $conn->prepare("UPDATE `suppliers` SET name = ?, phone = ?, email = ?, address = ? WHERE supplier_id = ?");
        $update_supplier_details->execute([$name, $phone, $email, $address, $id]);
        if($update_supplier_details->rowCount() > 0) {
        ...
        if(!empty($_POST['new_pass'])){ // Only update if new password is provided
            if($old_pass != $prev_password_db){
                ...
                $update_password = $conn->prepare("UPDATE `suppliers` SET password = ? WHERE supplier_id = ?");
            ...
            // No new password provided, just check if name/phone/email/address was updated
            if($update_supplier_details->rowCount() == 0) { // If details update didn't change anything
                 $message[] = 'No changes made or other error occurred!';
            }
        }
    }

    // After update, re-fetch the supplier details to display latest
    $select_current_supplier->execute([$supplier_id_to_update]);
    $fetch_current_supplier = $select_current_supplier->fetch(PDO::FETCH_ASSOC);
}
?>
...
<head>
    ...
    <title>FashionGuys | Update Supplier</title>
...
<section class="update-supplier mt-5">
    <div class="content-card">
        <div class="card-header">
            <div class="header-icon">
                <i class="fas fa-industry"></i>
            </div>
            <h2 class="header-title">Update Supplier: <?= htmlspecialchars($fetch_current_supplier['name']); ?></h2>
            <p class="text-muted">Modify supplier details or change password</p>
        </div>
...
<?php
        ...
        <form action="" method="post" class="needs-validation" novalidate>
            <input type="hidden" name="id" value="<?= htmlspecialchars($fetch_current_supplier['supplier_id']); ?>">
            <div class="mb-4">
                <div class="form-floating">
                    <input type="text"
                           ...
                           value="<?= htmlspecialchars($fetch_current_supplier['name']); ?>"
                           required
                           maxlength="18"
                           oninput="this.value = this.value.replace(/\s{2,}/g, ' ')">
                    <label for="name">Full Name</label>
                    <div class="invalid-feedback">
                        Please enter the supplier's full name (max 18 characters)
                    </div>
                </div>
            </div>
            ...
            <div class="mb-4">
                <div class="form-floating">
                    <input type="tel"
                           ...
                           value="<?= htmlspecialchars($fetch_current_supplier['phone']); ?>"
                           required
                           pattern="[0-9]{10}"
                           maxlength="10"
            ...
            <div class="mb-4">
                <div class="form-floating">
                    <input type="email"
                           ...
                           value="<?= htmlspecialchars($fetch_current_supplier['email']); ?>"
                           required
                           maxlength="40">
                    <label for="email">Email Address</label>
                    <div class="invalid-feedback">
                        Please enter a valid email address (max 40 characters)
                    </div>
                </div>
            </div>
            ...
            <div class="mb-4">
                <div class="form-floating">
                    <textarea id="address"
                              ...
                              required
                              maxlength="100"
                              rows="3"><?= htmlspecialchars($fetch_current_supplier['address']); ?></textarea>
                    <label for="address">Address (Max 100 characters)</label>
                    <div class="invalid-feedback">
                        Please enter the full address (max 100 characters)
                    </div>
                </div>
            </div>
            ...
            <button type="submit"
                ...
                <i class="fas fa-check-circle me-2"></i>Update Supplier
            </button>
