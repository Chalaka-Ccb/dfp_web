...
if(isset($_POST['delete_manager'])){
    $delete_id = $_POST['manager_id'];
    $delete_id = filter_var($delete_id, FILTER_SANITIZE_STRING);

    $delete_manager = $conn->prepare("DELETE FROM `product_manager` WHERE manager_id = ?");
    $delete_manager->execute([$delete_id]);

    if($delete_manager->rowCount() > 0){
        $message[] = 'Product Manager deleted successfully!';
    } else {
        $message[] = 'Failed to delete Product Manager or manager not found!';
    }
}

// Fetch all product managers
$select_managers = $conn->prepare("SELECT manager_id, name, expertise FROM `product_manager`");
$select_managers->execute();
$managers = $select_managers->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>FashionGuys | View Product Managers</title>
...
<section class="view-product-managers mt-5">
    <div class="content-card">
        <div class="card-header">
            <div class="header-icon">
                <i class="fas fa-cubes"></i> </div>
            <h2 class="header-title">Manage Product Managers</h2>
            <p class="text-muted">View, update, or remove existing Product Manager accounts</p>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle table-custom">
                ...
                <tbody>
                        ...
                        <?php foreach ($managers as $manager): ?>
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="update_product_manager.php?id=<?= $manager['manager_id']; ?>" class="btn btn-action btn-edit me-2">
                                        <i class="fas fa-edit"></i> Update
                                    </a>
                                    <form action="" method="post" style="display:inline-block;" onsubmit="return confirm('Are you sure you want to delete this Product Manager?');">
                                        <input type="hidden" name="manager_id" value="<?= $manager['manager_id']; ?>">
                                        <button type="submit" name="delete_manager" class="btn btn-action btn-delete">
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="4" class="text-center py-4">No Product Managers found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
