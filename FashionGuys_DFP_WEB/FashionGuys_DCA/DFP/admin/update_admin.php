...
if(!$user_id_to_update){
    // If no ID is provided, redirect back to view_admins or show an error
    header('location:view_admins.php');
    exit();
}

// Fetch current admin details
$select_current_admin = $conn->prepare("SELECT id, name FROM `admins` WHERE id = ?");
$select_current_admin->execute([$user_id_to_update]);
$fetch_current_admin = $select_current_admin->fetch(PDO::FETCH_ASSOC);

if(!$fetch_current_admin){
    // Admin not found, redirect
...
if(isset($_POST['update_admin'])){
    ...
    $prev_pass_check = $conn->prepare("SELECT password FROM `admins` WHERE id = ?");
    ...
    $select_existing_name = $conn->prepare("SELECT id FROM `admins` WHERE name = ? AND id != ?");
    ...
    if($select_existing_name->rowCount() > 0){
        ...
        if(!empty($_POST['new_pass'])){ // Only update if new password is provided
            ...
            // No new password provided, just check if name was updated
            if($update_name->rowCount() == 0) { // If username update didn't change anything
                 $message[] = 'No changes made or other error occurred!';
            }
        }
    }

    // After update, re-fetch the admin details to display latest
    $select_current_admin->execute([$user_id_to_update]);
    $fetch_current_admin = $select_current_admin->fetch(PDO::FETCH_ASSOC);
}
?>
...
<head>
    ...
    <title>FashionGuys | Update Admin</title>
...
<section class="update-admin mt-5">
    <div class="content-card">
        <div class="card-header">
            <div class="header-icon">
                <i class="fas fa-user-edit"></i>
            </div>
            <h2 class="header-title">Update Admin: <?= htmlspecialchars($fetch_current_admin['name']); ?></h2>
            <p class="text-muted">Modify admin details or change password</p>
        </div>
        ...
        <form action="" method="post" class="needs-validation" novalidate>
            <input type="hidden" name="id" value="<?= htmlspecialchars($fetch_current_admin['id']); ?>">
            <div class="mb-4">
                <div class="form-floating">
                    <input type="text"
                           ...
                           value="<?= htmlspecialchars($fetch_current_admin['name']); ?>"
                           required
                           maxlength="20"
            ...
            <button type="submit"
                ...
                <i class="fas fa-check-circle me-2"></i>Update Admin
            </button>
            <div class="text-center mt-3">
                <a href="view-admin.php" class="btn btn-secondary">
