...
if(isset($_POST['delete_admin'])){
    ...
    // Prevent deleting the currently logged-in admin (optional but recommended)
    if($delete_id == $admin_id){
        ...
        if($delete_admin->rowCount() > 0){
            ...
            $message[] = 'Failed to delete admin or admin not found!';
        }
    }
}

// Initialize $message as an empty array if not already set by delete action
if(!isset($message)){
    $message = [];
}

// Fetch all admins
$select_admins = $conn->prepare("SELECT id, name FROM `admins`");
...
<head>
    ...
    <title>FashionGuys | View Owners</title>
...
<section class="view-admins mt-5">
    <div class="content-card">
        <div class="card-header">
            ...
            <p class="text-muted">View, update, or remove existing administrators</p>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle table-custom">
                ...
                <tbody>
                        ...
                        <?php foreach ($admins as $admin): ?>
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="update_admin.php?id=<?= $admin['id']; ?>" class="btn btn-action btn-edit me-2">
                                        <i class="fas fa-edit"></i> Update
                                    </a>
                                    <form action="" method="post" style="display:inline-block;" onsubmit="return confirm('Are you sure you want to delete this admin?');">
                                        <input type="hidden" name="admin_id" value="<?= $admin['id']; ?>">
                                        <button type="submit" name="delete_admin" class="btn btn-action btn-delete">
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="3" class="text-center py-4">No admin accounts found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
